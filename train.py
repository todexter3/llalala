...
def train_system(data_path: str = "data/AA_Comdty_cpu",
    ...
    for ep in range(n_episodes):
        ...
        if (ep+1) % save_interval == 0:
            agent.save(f'checkpoint_ep{ep+1}.pth')
    ...
    # ---------- Baseline: TWAP (taker at L1) ----------
    baseline = simulate_twap_taker(test_env.processed_data, config)
    ...
    print(f"  TWAP- Completion: {baseline['completion']*100:.2f}% | "
          f"Avg Cost: {baseline['avg_cost_bps']:.2f} bps | Shortfall(VWAP): {baseline['shortfall_bps']:.2f} bps | "
          f"PV: {baseline['pv']:.2f} | Trades: {baseline['num_trades']}")

    create_visualization(history, test_env, config, baseline)
